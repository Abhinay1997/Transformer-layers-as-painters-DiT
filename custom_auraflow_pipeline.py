...
class AuraFlowPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        """
        #
        transformer_encoder_activations_over_time = None
        transformer_activations_over_time = None
        single_transformer_activations_over_time = None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred, transformer_encoder_activations, transformer_activations, single_transformer_activations = self.transformer(
                    latent_model_input,
                ...
                if track_activations:
                    if transformer_encoder_activations_over_time is None:
                        transformer_encoder_activations_over_time = torch.zeros_like(transformer_encoder_activations)
                        transformer_activations_over_time = torch.zeros_like(transformer_activations)
                        single_transformer_activations_over_time = torch.zeros_like(single_transformer_activations)
                        
                    transformer_encoder_activations_over_time = torch.add(transformer_encoder_activations_over_time, transformer_encoder_activations)
                    transformer_activations_over_time = torch.add(transformer_activations_over_time, transformer_activations)
                    single_transformer_activations_over_time = torch.add(single_transformer_activations_over_time,single_transformer_activations)
        ...
        if track_activations:
            return ImagePipelineOutput(images=image), torch.div(transformer_encoder_activations_over_time, num_inference_steps), torch.div(transformer_activations_over_time,num_inference_steps), torch.div(single_transformer_activations_over_time,num_inference_steps)
        return ImagePipelineOutput(images=image), None, None, None