...
class AuraFlowTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        # Capture hidden state activations
        joint_encoder_activations = []
        joint_activations = []
        single_transformer_activations = []
        ...
        transformer_block_map = {i:layer for (i,layer) in enumerate(self.joint_transformer_blocks)}
        reordered_transformer_blocks = [transformer_block_map[i] if not isinstance(i, tuple) else i for i in layer_order]
        # MMDiT blocks.
        for index_block, block in enumerate(reordered_transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                if isinstance(block, tuple):
                    accumulate_hidden_states = None
                    accumulate_encoder_hidden_states = None
                    print(f"Running parallel loop for layers {block}")
                    for block_idx in block:
                        tmp_encoder_hidden_states, tmp_hidden_states = transformer_block_map[block_idx](
                            hidden_states=hidden_states, encoder_hidden_states=encoder_hidden_states, temb=temb
                        )
                        if accumulate_hidden_states is None:
                            accumulate_hidden_states = torch.zeros_like(tmp_hidden_states)
                            accumulate_encoder_hidden_states = torch.zeros_like(tmp_encoder_hidden_states)
                        accumulate_hidden_states = torch.add(accumulate_hidden_states, tmp_hidden_states)
                        accumulate_encoder_hidden_states = torch.add(accumulate_encoder_hidden_states, tmp_encoder_hidden_states)
                    encoder_hidden_states = torch.div(accumulate_encoder_hidden_states, len(block))
                    hidden_states = torch.div(accumulate_hidden_states, len(block))
                else:
                ...
                if track_activations:
                    joint_encoder_activations.append(encoder_hidden_states)
                    joint_activations.append(hidden_states)

        single_transformer_block_map = {i:layer for (i,layer) in enumerate(self.single_transformer_blocks)}
        reordered_single_transformer_blocks = [single_transformer_block_map[i] if not isinstance(i, tuple) else i for i in single_layer_order]
        ...
        if len(self.single_transformer_blocks) > 0:
            ...
            for index_block, block in enumerate(reordered_single_transformer_blocks):
                if self.training and self.gradient_checkpointing:
                    ...
                    if isinstance(block, tuple):
                        accumulate_hidden_states = []
                        print(f"Running parallel loop for single layers {block}")
                        for block_idx in block:
                            tmp_hidden_states = single_transformer_block_map[block_idx](
                                hidden_states=combined_hidden_states, temb=temb
                            )
                            accumulate_hidden_states.append(tmp_hidden_states)
                        combined_hidden_states = torch.mean(torch.stack(accumulate_hidden_states), dim=0)
                    else:
                        combined_hidden_states = block(hidden_states=combined_hidden_states, temb=temb)

                    if track_activations:
                        single_transformer_activations.append(combined_hidden_states)
        ...
        if not return_dict:
            return (output,)
        if track_activations:
            joint_activations = torch.stack(joint_activations)
            joint_encoder_activations = torch.stack(joint_encoder_activations)
            single_transformer_activations = torch.stack(single_transformer_activations)
        return output, joint_encoder_activations, joint_activations, single_transformer_activations